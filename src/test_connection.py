...
def test_connection():
    ...
    print("📝 環境変数の確認:")
    print(
        f'  - DISCORD_TOKEN: {"設定済み" if token else "未設定"} '
        f"(長さ: {len(token) if token else 0})"
    )
    print("  - TARGET_GUILD_ID: 設定済み")
    print()
    ...
    async def on_ready():
        ...
        try:
            ...
            if show_details:
                ...
                detailed_info["member_count"] = (
                    guild.member_count if guild.member_count else "不明"
                )
