# GitHub Actions ワークフロー詳細ドキュメント

このドキュメントでは、リポジトリで使用されているGitHub Actionsワークフローの詳細な仕様と動作を説明します。

クイックリファレンスについては [.github/workflows/README.md](../.github/workflows/README.md) を参照してください。

## 目次

- [知識データの生成と保存](#知識データの生成と保存)
- [Discord Botの実行](#discord-botの実行)
- [Auto Merge on Approval](#auto-merge-on-approval)
- [Auto Delete Branch on Merge](#auto-delete-branch-on-merge)
- [Update Other PRs After Merge](#update-other-prs-after-merge)
- [Secrets疎通テスト](#secrets疎通テスト)

## 知識データの生成と保存

**ワークフローファイル**: `generate-knowledge-data.yml`
...
- 定期実行（スケジュール設定は[ワークフローファイル](../.github/workflows/generate-knowledge-data.yml)を参照）
...
- タイムアウト設定: 60分（[ワークフローファイル](../.github/workflows/generate-knowledge-data.yml)を参照）
...
**ワークフローファイル**: `run-discord-bot.yml`
...
- 手動実行（`workflow_dispatch`）
  - 実行理由（任意）
  - 実行時間上限（オプション詳細は[ワークフローファイル](../.github/workflows/run-discord-bot.yml)を参照）
...
- 実行時間の上限はワークフローの設定で選択可能（[ワークフローファイル](../.github/workflows/run-discord-bot.yml)を参照）
- 知識データのReleaseが存在しない場合はエラーになります
- 「知識データの生成と保存」ワークフローを先に実行してください

## Auto Merge on Approval

**ワークフローファイル**: `auto-merge.yml`
...
**ワークフローファイル**: `auto-delete-branch.yml`
...
## Update Other PRs After Merge

**ワークフローファイル**: `update-other-prs.yml`
