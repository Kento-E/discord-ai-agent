...
**✨ 新機能**: 過去のメッセージから学習したペルソナを基に、入力に対する予測返信を自動生成します。外部AIサービスを使わず、完全無料で稼働できます。
...
- `!ask <質問内容>`: 質問に対して、過去の知識とペルソナに基づいた予測返信を生成
- `@Bot <質問内容>`: Botへのメンションでも同様に応答

**応答の特徴**:
- 過去メッセージから学習したペルソナに基づいて返信を生成
- 入力の種類（質問・挨拶・通常会話）を自動判別
- ペルソナの文末表現や語彙を活用した自然な返信
