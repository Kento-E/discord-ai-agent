...
承認されたPRは自動的にマージされます（`auto-merge.yml`）。

### ブランチ自動削除

マージされたPRのブランチは自動的に削除されます（`auto-delete-branch.yml`）。

### 他のPRの自動更新

PRがmainブランチにマージされると、同じベースブランチを対象とする他のオープンなPRが自動的に更新されます（`update-other-prs.yml`）。

**機能の詳細**：

- mainブランチへのPRマージ時に自動的にトリガーされます
- 同じベースブランチ（main）を対象とする他のすべてのオープンなPRを検出します
- 各PRのブランチに最新のmainブランチをマージして更新します
- コンフリクトがある場合はスキップし、エラーレポートを出力します
- 複数PRがある場合の手動更新作業を削減できます
